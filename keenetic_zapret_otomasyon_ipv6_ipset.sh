...
case "$1" in
    ...
    --telegram-daemon)
        ...
        echo "$_self_pid" > /tmp/zkm_telegram_bot.pid
        # Tum eski telegram-daemon processleri oldur
        ...
        # Telegram sunucusunun eski oturumu kapatmasi icin bekle (409 onlemi)
        sleep 5
        ZKM_SKIP_LOCK="1"
        ;;
...
_zkm_restart_services_after_update() {
    ...
    if [ -d "$KZM_GUI_DIR" ]; then
        ...
        (export ZKM_SKIP_LOCK=1; sh "/opt/lib/opkg/keenetic_zapret_otomasyon_ipv6_ipset.sh" --update-gui </dev/null >/dev/null 2>&1)
...
if [ "$_force" != "1" ] && ! ver_is_newer "$REMOTE_FILE_VER" "$LOCAL_VER"; then
    ...
    printf '%b%s%b\n' "${CLR_GREEN}${CLR_BOLD}" "$(T _ '  Cikis yapip KZM yi yeniden calistirin.' '  Please exit and re-run KZM.')" "${CLR_RESET}"
    print_line "="
    echo ""
    return 0
}
