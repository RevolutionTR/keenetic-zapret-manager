...
healthmon_updatecheck_do() {
    ...
    if [ "$upd_mode" = "2" ]; then
        ...
        if update_manager_script >/tmp/zkm_autoupdate.log 2>&1; then
            ...
            # Tum telegram-daemon processleri durdur (telegram_bot_stop ile ayni mantik)
            # kill -9 ile tum orphan'lari temizle, watchdog yeniden baslatir
            ps 2>/dev/null | grep -- '--telegram-daemon' | grep -v grep |                 while IFS= read -r _pline; do
                    _ppid="$(printf '%s' "$_pline" | awk '{print $1}')"
                    [ -n "$_ppid" ] && kill -9 "$_ppid" 2>/dev/null
                done
            rm -f /tmp/zkm_telegram_bot.pid 2>/dev/null
            healthmon_log "$(date +%s 2>/dev/null) | updatecheck | zkm | tgbot_all_stopped will restart via watchdog"
...
# periodic update check (GitHub)
        ...
        # ---- ORPHAN BOT TEMIZLEME (her tur, watchdog'dan bagimsiz) ----
        _known_tg_pid="$(cat "$TG_BOT_PID_FILE" 2>/dev/null)"
        _all_tg_pids="$(ps 2>/dev/null | grep -- '--telegram-daemon' | grep -v grep | awk '{print $1}')"
        for _ppid in $_all_tg_pids; do
            [ -n "$_ppid" ] && [ "$_ppid" != "$_known_tg_pid" ] && {
                kill -9 "$_ppid" 2>/dev/null
                healthmon_log "$now | tgbot | orphan killed pid=$_ppid"
            }
        done
        ...
        if [ "${HM_TGBOT_WATCHDOG:-1}" = "1" ]; then
            ...
            if [ "$_tgbot_enable" = "1" ]; then
                ...
                # Orphan temizleme: pipeline degil for dongusu — subshell sorunu yok, kill -9
                _all_tg_pids="$(ps 2>/dev/null | grep -- '--telegram-daemon' | grep -v grep | awk '{print $1}')"
                for _ppid in $_all_tg_pids; do
                    [ -n "$_ppid" ] && [ "$_ppid" != "$_tgpid" ] && {
                        kill -9 "$_ppid" 2>/dev/null
                        healthmon_log "$now | tgbot_watchdog | orphan killed pid=$_ppid"
                    }
                done
                if [ -z "$_tgpid" ] || ! kill -0 "$_tgpid" 2>/dev/null; then
                    ...
                    sleep 1
                    _wd_pid="$(ps 2>/dev/null | awk '/--telegram-daemon/ && !/awk/{print $1}' | head -1)"
                    [ -n "$_wd_pid" ] && echo "$_wd_pid" > "$_tgpid_f" || echo $! > "$_tgpid_f"
                    healthmon_log "$now | tgbot_watchdog | bot started pid=${_wd_pid:-unknown}"
                fi
            fi
        fi
        # ---- WAN MONITOR ----
...
' "$_items" "$_rebind" ;;
    dns_add_preset)
        ...
        if [ "$_pkg_all_exist" -eq 1 ]; then
            ...
            ZKM_SKIP_LOCK=1 sh "$_kzm" --cgi-action dns_add_preset "$_pkg" >/dev/null 2>&1 &
