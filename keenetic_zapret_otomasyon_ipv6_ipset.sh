...
if [ "$_force" != "1" ] && ! ver_is_newer "$REMOTE_FILE_VER" "$LOCAL_VER"; then
    ...
    printf '%b%s%b\n' "${CLR_GREEN}${CLR_BOLD}" "$(T _ '  Degisikliklerin gorunmesi icin KZM yi yeniden calistirin:' '  Please re-run KZM to apply changes:')" "${CLR_RESET}"
    printf '%b%s%b\n' "${CLR_CYAN}${CLR_BOLD}" "    kzm" "${CLR_RESET}"
    print_line "="
    echo ""
    return 0
}
